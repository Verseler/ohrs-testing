...
<template>
    ...
    <AuthenticatedLayout>
        <div class="flex justify-between min-h-12">
            <Breadcrumb>
                <BreadcrumbList>
                    ...
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                        <BreadcrumbLink :href="route('reservation.show', { id: reservation.id })">
                            Reservation Details
                        </BreadcrumbLink>
                    </BreadcrumbItem>
                    <BreadcrumbSeparator />
        ...
        <Card>
            <CardHeader>
            <CardTitle class="text-xl font-bold">Process Payment</CardTitle>
            <CardDescription>
                Reservation #{{ reservation.reservation_code }} for {{ reservation.first_name }} {{ reservation.last_name }}
            </CardDescription>
            </CardHeader>
            ...
            <template  v-if="reservation.remaining_balance > 0">
                ...
                <div class="grid grid-cols-1 gap-4 mb-6 md:grid-cols-2">
                    <Card
                        ...
                        paymentOption === 'full' && 'border-primary-500'
                        ]"
                        @click="selectPaymentOption('full')"
                    >
                        <CardContent class="p-4 text-center">
                        <CheckCircleIcon v-if="paymentOption === 'full'" class="w-5 h-5 mx-auto mb-2 text-primary" />
                        <CircleIcon v-else class="w-5 h-5 mx-auto mb-2 text-muted-foreground" />
                        <h4 class="font-medium">Full Payment</h4>
                        <p class="text-sm text-muted-foreground">₱{{ formatCurrency(reservation.remaining_balance) }}</p>
                        </CardContent>
                    </Card>
                    ...
                    <Card
                        :class="[
                        'cursor-pointer border-2 transition-all',
                        paymentOption === 'custom' && 'border-primary-500'
                        ]"
                        @click="selectPaymentOption('custom')"
                    >
                        <CardContent class="p-4 text-center">
                        <CheckCircleIcon v-if="paymentOption === 'custom'" class="w-5 h-5 mx-auto mb-2 text-primary" />
                        <CircleIcon v-else class="w-5 h-5 mx-auto mb-2 text-muted-foreground" />
                        <h4 class="font-medium">Custom Amount</h4>
                        <p class="text-sm text-muted-foreground">Specify amount</p>
                        </CardContent>
                    </Card>
                </div>
                ...
                <div v-if="form.errors.reservation_id" class="text-red-500">
